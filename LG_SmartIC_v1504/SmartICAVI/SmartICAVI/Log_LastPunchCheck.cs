using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace SmartICAVI
{
    class Log_LastPunchCheck
    {
        static LogUtill log = new LogUtill("LastPunchCheck");
        //static string logHead = "[Trace]    ";

        public delegate void LogEventHandler(string log);
        public static event LogEventHandler LogEvent;

        public static int Type
        {
            get
            {
                return log.Type;
            }
            set
            {
                log.Type = value;
            }
        }

        public static string Path
        {
            get
            {
                return log.Path;
            }
            set
            {
                log.Path = value;
            }
        }

        public static int SaveDays
        {
            get
            {
                return log.SaveDays;
            }
            set
            {
                log.SaveDays = value;
            }

        }

        public static void WriteLine(string content)
        {
            string logdata = string.Format("{0}", content);
            log.WriteLine(logdata);
            //Log.WriteLine(logHead + logdata);

            if (null != LogEvent)
                LogEvent(logdata);
        }

        public static void WriteLine(string formatString, params object[] args)
        {
            WriteLine(string.Format(formatString, args));
        }

        public static void Open()
        {
            log.Open();
        }

        public static void Close()
        {
            log.Close();
        }
    }
}
